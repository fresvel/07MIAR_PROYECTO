...
@dataclass
class TrainerConfig:
    loader: str = "gen"   # "gen" o "tf"
    img_size: Tuple[int, int] = (224, 224)
    batch_size: int = 32
    epochs: int = 50
    mode: str = "scratch"                     # "scratch" o "transfer" (solo afecta augmentación del loader)
    learning_rate: float = 1e-3
    val_size: float = 0.15
    test_size: float = 0.15
    seed: int = 42
    model_out: str = "model_scratch_best.keras"
    patience_es: int = 7                      # EarlyStopping patience
    patience_rlrop: int = 3                   # ReduceLROnPlateau patience
    min_lr: float = 1e-6
    save_dir: str = "./results"                       # carpeta donde guardar el mejor modelo
    num_classes: int = 3


class LemonTrainerGen:
    """
    Entrenador POO usando ImageDataGenerator + Adam para la estrategia 'desde cero'.

    Flujo:
      prepare_data() -> build_model() -> compile_model() -> train() -> evaluate() -> plot_history()
      run_full_experiment() ejecuta todo lo anterior en orden.
    """
    def __init__(self, config: Optional[TrainerConfig] = None):
        self.cfg = config or TrainerConfig()
        self.loader: Optional[LemonGenLoader] = None
        self.train_gen = None
        self.val_gen = None
        self.test_gen = None

        self.builder: Optional[LemonCNNBuilder] = None
    ...
    # Datos
    # -----------------------------
    def prepare_data(self, val_size: float = 0.15, test_size: float = 0.15, seed: int = 42):
        """
        Crea generadores de train/val/test con augmentación definida por 'mode'.
        """
        ...
        # _create_splits dentro del loader ya fue llamado en su __init__; si deseas sobreescribir proporciones:
        self.loader._create_splits(val_size=val_size, test_size=test_size, seed=seed)
        self.train_gen, self.val_gen, self.test_gen = self.loader.get_generators()
        return self
    ...
    def build_model(self):
        """
        Construye el modelo Functional (SeparableConv) con Softmax final.
        """
    ...
    def train(self):
        """
        Ejecuta el entrenamiento y guarda el mejor modelo (por val_loss).
        """
    ...
    def evaluate(self) -> Dict[str, Any]:
        """
        Evalúa en test; retorna dict con métricas.
        """
        test_loss, test_acc = self.model.evaluate(self.test_gen, verbose=0)
        return {"test_loss": float(test_loss), "test_accuracy": float(test_acc), "best_model_path": self.best_model_path}
    ...
    def plot_history(self):
        """
        Grafica curvas de pérdida y accuracy (para el informe).
        """
